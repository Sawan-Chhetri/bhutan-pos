...
export async function GET(request, { params }) {
  try {
    ...
    const storeData = storeSnap.exists
      ? storeSnap.data()
      : { name: "Unknown Store", address: "", phone: "", gstNumber: "" };
    ...
    return NextResponse.json({
      ...
      // REFUND DATA (NEW)
      gstRefunded: gstData.gstRefunded || 0,
      refundCount: gstData.refundCount || 0,
      ...
      business: {
        ...
        phone: storeData.phone || "",
        gstNumber: storeData.gstNumber || storeData.tpn || "TPN PENDING", // Check both field names
      },
    });
  } catch (err) {
    console.error("GST report fetch error:", err);
