...
export const syncInventory = async (idToken, storeId, force = false) => {
  try {
      ...
      try {
        ...
        if (vRes.ok) {
          ...
          if (serverVersion > localVersion) {
            console.log(
              `[SmartSync] 📣 New content available (Server: ${serverVersion} > Local: ${localVersion}). Syncing...`,
            );
            shouldSync = true;
          } else {
            ...
            if (count === 0) {
              console.log(
                `[SmartSync] ⚠️ Versions match but DB is empty. Forcing Sync.`,
              );
              shouldSync = true;
            } else {
              console.log(
                `[SmartSync] ✅ Local cache up-to-date (Version: ${localVersion}). Skipping download.`,
              );
              return count;
            }
          }
    ...
    const storedLastSync = localStorage.getItem(
      `swiftgst_last_sync_${storeId}`,
    );
    ...
    console.log(
      `[SmartSync] 🔄 Sync Initiated (Mode: ${effectiveSince > 0 ? "Delta" : "Full"})`,
    );
